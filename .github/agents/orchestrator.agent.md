---
name: orchestrator
description: 'Creates a task decomposition and implement it taking a task from BACKLOG using subagents.'
...
Invoke: **analyst** (#file:./analyst.agent.md ) — attach only the backlog task id.
...
If the analyst returns `complexity: "high"`: invoke **design** (#file:./design.agent.md ) — pass `tasks_file` from Step 1. Store the returned `design_file` and pass it to the implementation agent in Step 4.
Skip this step if `complexity` is `"low"` or `"medium"`.

### Step 3 — Branch creation (MANDATORY)
Invoke: **git** (#file:./git.agent.md ) — pass the `branch` value from Step 1, operation `CREATE_BRANCH`.
Do NOT proceed to Step 4 until the git agent returns `{ "status": "success" }`.

### Step 4 — Implementation (MANDATORY)
  a. Invoke: **implementation** (#file:./implementation.agent.md ) — pass `tasks_file` from Step 1, the confirmed branch name from Step 3, and `design_file` if produced in Step 2.
  b. Invoke: **review** (#file:./review.agent.md ) — pass `design_file` if produced in Step 2. This step is NOT optional even if the implementation appears correct.
      → If review returns `{ "status": "fail" }`: re-invoke **implementation** (#file:./implementation.agent.md ) with the `findings` array from the review output (max 2 retries).
...
Invoke: **git** (#file:./git.agent.md ) — operation `UPDATE_CHANGELOG`, pass the task id or ids and a one-line summary. This step MUST run even if it seems like minor work.
